...
class LAFNet_CVPR2019(nn.Module):
    def __init__(self, modal_type=None, use_cost=False):
        ...
        if modal_type is not None:
            self.modal_conv1 = nn.Conv2d( 1, 64, kernel_size=3, padding=1)
            self.modal_bn1 = nn.BatchNorm2d(64)
            self.modal_conv2 = nn.Conv2d(64, 64, kernel_size=3, padding=1)
            self.modal_bn2 = nn.BatchNorm2d(64)
            self.modal_conv3 = nn.Conv2d(64, 64, kernel_size=1, padding=0)
            self.modal_bn3 = nn.BatchNorm2d(64)
            
            self.modal_att_conv1 = nn.Conv2d(64, 64, kernel_size=3, padding=1)
            self.modal_att_bn1 = nn.BatchNorm2d(64)
            self.modal_att_conv2 = nn.Conv2d(64,  1, kernel_size=1, padding=0)    
            self.modal_att_bn2 = nn.BatchNorm2d(1)
        ...
        # embedding
        if (use_cost) and (modal_type is not None):
            self.embed_conv1 = nn.Conv2d(256, 64, kernel_size=3, padding=1)
        elif ((use_cost) and (modal_type is None)) or ((not use_cost) and (modal_type is not None)):
            self.embed_conv1 = nn.Conv2d(192, 64, kernel_size=3, padding=1)
        elif (not use_cost) and (modal_type is None):
            self.embed_conv1 = nn.Conv2d(128, 64, kernel_size=3, padding=1)
    ...
    def forward(self, cost, disp, imag, modal=None):
        # feature extraction networks
        if self.use_cost:
            # x = self.softmax(-self.L2normalize(cost)*100)
            # x = torch.topk(x, k=7, dim=1).values
            x = F.relu(self.cost_bn1(self.cost_conv1(cost)))
        ...
        if self.modal_type is not None:
            x = F.relu(self.modal_bn1(self.modal_conv1(modal)))
            x = F.relu(self.modal_bn2(self.modal_conv2(x)))
            modal_x = F.relu(self.modal_bn3(self.modal_conv3(x)))
        ...
        if self.modal_type is not None:
            x = F.relu(self.modal_att_bn1(self.modal_att_conv1(modal_x)))
            modal_att_x = self.modal_att_bn2(self.modal_att_conv2(x))
        
        if (self.use_cost) and (self.modal_type is not None):
            att_x = self.softmax_att(torch.cat((cost_att_x,disp_att_x,imag_att_x,modal_att_x),1))

            cost_att_x = att_x[:,0,:,:].unsqueeze(1)
            disp_att_x = att_x[:,1,:,:].unsqueeze(1)
            imag_att_x = att_x[:,2,:,:].unsqueeze(1)
            modal_att_x = att_x[:,3,:,:].unsqueeze(1)

            cost_x = cost_x*(cost_att_x.repeat(1,64,1,1))
            disp_x = disp_x*(disp_att_x.repeat(1,64,1,1))
            imag_x = imag_x*(imag_att_x.repeat(1,64,1,1))
            modal_x = modal_x*(modal_att_x.repeat(1,64,1,1))

            x = torch.cat((cost_x,disp_x,imag_x,modal_x),1)
            ...
            att_x = self.softmax_att(torch.cat((disp_att_x,imag_att_x,modal_att_x),1))

            disp_att_x = att_x[:,0,:,:].unsqueeze(1)
            imag_att_x = att_x[:,1,:,:].unsqueeze(1)
            modal_att_x = att_x[:,2,:,:].unsqueeze(1)

            disp_x = disp_x*(disp_att_x.repeat(1,64,1,1))
            imag_x = imag_x*(imag_att_x.repeat(1,64,1,1))
            modal_x = modal_x*(modal_att_x.repeat(1,64,1,1))

            x = torch.cat((disp_x,imag_x,modal_x),1)
        elif (not self.use_cost) and (self.modal_type is None):
            att_x = self.softmax_att(torch.cat((disp_att_x,imag_att_x),1))
